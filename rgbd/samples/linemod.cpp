...
int main(int argc, char * argv[])
{
  ...
  int num_classes = 0;
  int matching_threshold = 75;
  /// @todo Keys for changing these?
  cv::Size roi_size(250, 300);
  ...
  // std::string dataset_root = "/media/aumero/Data/Dataset/UE_NDDS/owl_for_linemod_train/";
  std::string dataset_root = "/home/aumero/extension_aumero_ws/realsense_record_2020-02-13-50-41/";

  double focal_length = 610;

  int frame_cnt = 0;

  // Main loop
  cv::Mat color, depth, gt_mask;
  for(;;)
  {
    ...
    std::vector<cv::Mat> sources;
    color = cv::imread(dataset_root+"bgr_"+std::to_string(frame_cnt)+".png");
    depth = cv::imread(dataset_root+"depth_"+std::to_string(frame_cnt)+".png",cv::IMREAD_ANYDEPTH);
    // std::string index_str = cv::format("%06d",frame_cnt);
    // color = cv::imread(dataset_root+"/"+index_str+".png");
    // depth = cv::imread(dataset_root+"/"+index_str+".depth.mm.16.png",cv::IMREAD_ANYDEPTH);
    // gt_mask = cv::imread(dataset_root+"/"+index_str+".cs.png");
    frame_cnt++;
    if(frame_cnt > 2000)
      frame_cnt = 0;
    sources.push_back(color);
...
static void filterPlane(IplImage * ap_depth, std::vector<IplImage *> & a_masks, std::vector<CvPoint> & a_chain, double f)
{
  ...
  for (int l_i = 0; l_i < (int)l_chain_vector.size(); ++l_i)
  {
    ...
    // std::cout << "lp_src_3Dpts.z : " << lp_src_3Dpts[l_i].z << std::endl;
